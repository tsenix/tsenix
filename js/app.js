(() => {
    ...
    function initSliders() {
        if (document.querySelector(".feedbacks__body")) new Swiper(".feedbacks__body", {
            modules: [ Pagination ],
            observer: true,
            observeParents: true,
            speed: 800,
        ...
        menuButton.addEventListener("click", (() => {
            if (!document.documentElement.classList.contains("menu-open")) {
                ...
                for (let i = 0; i < menuItems.length; i++) {
                    ...
                    menuItem.style.transitionDelay = `${.1 * i + .3}s`;
                }
            } else {
                ...
                for (let i = 0; i < menuItems.length; i++) {
                    const menuItem = menuItems[i];
                    menuItem.style.transitionDelay = "0.5s";
                }
                menuBody.scrollTop = 0;
            }
        }));
    }
    function animateCounters(counters, duration = 2e3) {
        Math.max(...counters.map((c => +c.dataset.target)));
    ...
    const block = document.querySelector(".main__statistic");
    if (block) {
        const counters = [ ...block.querySelectorAll(".main__statistic-item-count span") ];
        const observer = new IntersectionObserver((entries => {
            entries.forEach((entry => {
                if (entry.isIntersecting) animateCounters(counters, 2e3); else counters.forEach((c => c.textContent = "0"));
            }));
        }), {
            threshold: .5
        });
    ...
    const slider = document.querySelector(".platforms__body");
    let isDown = false;
    let startX;
    let scrollLeft;
    if (slider) {
